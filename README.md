...
Ran clean-slate `/pr-war-stories setup --limit 500` + `/pr-war-stories harvest` on three production repos. Same methodology for all three; uniform measurements. Full artifacts (every BUGBOT.md, every LESSON, every MINING_LOG) in [`docs/case-studies/`](docs/case-studies/).

| Repo | Stack | PRs mined | BUGBOT files | Rules | Lessons | % from dismissals | Tokens | Wall time |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| [octostar-frontend](docs/case-studies/octostar-frontend/REPORT.md) | React + TS monorepo | 415 | 5 | 39 | 10 | 36% | 150,509 | ~17 min |
| [octostar-api](docs/case-studies/octostar-api/REPORT.md) | FastAPI + Python | 500 | 8 | 46 | 10 | 30% | 138,899 | ~12 min |
| [ReadonlyREST](docs/case-studies/readonlyrest/REPORT.md) | Scala / JVM (OSS) | 755 | 7 | 27 | 7 | 52% | 175,023 | ~77 min |
| **Total / avg** | | **1,670** | **20** | **112** | **27** | **~39%** | **464,431** | **~106 min** |

**Key findings replicated across all three repos:**

- **Author-dismissals are the primary harvest yield.** 30–52% of all rules trace directly to `"Dismissed — intentional because X"` replies authors wrote to bot reviewers. This is true across TypeScript/React (Bugbot), Python/FastAPI (Bugbot + Copilot), and Scala/JVM (coderabbitai) stacks. The v0.7 doctrine prioritising these is load-bearing.
- **Density-based hierarchy beats intuition.** All three repos had module-level scopes the skill correctly identified by comment density. In octostar-frontend, `components/Apps/` alone carried 442 of 1,067 human review comments (41%) — invisible to intuition-based scoping.
- **The GitHub Copilot filter bug is real.** On octostar-api, 308 Copilot Code Review comments (36% of "apparently human" traffic) would have been misclassified under the pre-v0.7 filter. Fixed in the shipped template before any public install.

## What it costs

Measured empirically across the three case studies above. All three are "one-shot" runs: a fresh Claude Code session executing setup + harvest end-to-end.

| Metric | octostar-frontend | octostar-api | ReadonlyREST |
|---|---:|---:|---:|
| Claude Code tokens | 150,509 | 138,899 | 175,023 |
| Wall time | ~17 min | ~12 min | ~77 min |
| `gh` API calls | ~421 | 501 | ~195 |
| Approx Sonnet 4.6 cost (no caching) | ~$0.72 | ~$0.66 | ~$0.83 |
| Approx Sonnet 4.6 cost (with prompt caching) | **~$0.25–0.50** | **~$0.25–0.50** | **~$0.30–0.60** |

**The GitHub Action is free.** `harvest-lessons.yml` is pure JavaScript running in GitHub Actions — it fires on every merged PR, extracts substantive human review comments, posts a harvest summary as a PR comment, and **never invokes Claude**. You only spend tokens when *you* run the slash-commands interactively (`/pr-war-stories setup`, `/pr-war-stories harvest`, `/pr-war-stories audit`, `/pr-war-stories rebalance`, `/pr-war-stories recheck`).

Steady-state cost on an actively-reviewed repo: 1–2 harvest invocations per week at ~10k–30k tokens each (~$0.03–$0.15 per run). The expensive one is the initial setup; after that you're buying opinionated classification help for pennies per merged PR.
